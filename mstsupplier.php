    ...
    <div class="ps-3">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 p-0">
                ...
                <li><i class="fa-solid fa-industry"></i> Supplier</li>
            </ol>
        </nav>
    </div>
    <div class="ms-auto">
    ...
    <div class="col-12 col-lg-12">
        <div class="card shadow-white">
            <div class="card-header p-4">
                ...
                <button type="button" id="addspl" data-id="tambah" class="btn btn-secondary waves-effect raised"><i class="fa-solid fa-plus-circle"></i> Add</button>
            </div>
            <div class="card-body p-4">
                <table id="dataspl" class="table table-hover" width="100%">
                    <thead class="bg-cyan-400 font-weight-bold" style="height: 40px;">
                        <tr>
                            ...
                            <th width="3%">Phone</th>
                            <th width="3%">Taxable</th>
                            <th width="3%">Tax/NPWP</th>
                            <th width="6%">Opsi</th>
                        </tr>
                    </thead>
                    <tbody id="splbody">
                        ...
                        include("koneksi.php");
                        $i = 1;
                        $sql = "SELECT * FROM supplier ORDER BY nmspl ASC";
                        $ress = mysqli_query($conn1, $sql);

                        while ($data = mysqli_fetch_array($ress)) {
                            echo '<tr>';
                            echo '<td class="text-center">' . $i . '</td>';
                            echo '<td class="text-start">' . $data['nmspl'] . '</td>';
                            echo '<td class="text-start">' . $data['almtspl'] . '</td>';
                            echo '<td class="text-start">' . $data['telpspl'] . '</td>';
                            echo '<td class="text-start">' . $data['pkp'] . '</td>';
                            echo '<td class="text-start">' . $data['npwp'] . '</td>';
                        ?>
                            <td>
                                <button id="editspl" data-id="<?php echo $data['idspl']; ?>" class="btn btn-primary raised"><i class="fa-solid fa-pencil"></i></button>
                                <button id="delespl" data-id="<?php echo $data['idspl']; ?>" class="btn btn-warning raised"><i class="fa-solid fa-trash"></i></button>
                            </td>
                        <?php echo '</td>';
                            echo '</tr>';
                            $i++;
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#dataspl').parent().addClass("table-responsive");
    });

    // Menampilkan Add data modal
    $(document).on('click', '#addspl', function(e) {
        e.preventDefault();
        $("#ScrollableModal").modal('show');
        $.post('mstsupplier_add.php', {
            idspl: $(this).attr('data-id'),
            btnproses: 'Save'
        }, function(html) {
            $('#formmodal')[0].reset();
            $('#form_mode').val('add');
    ...
    $(document).on('click', '#editspl', function(e) {
        e.preventDefault();
        $("#ScrollableModal").modal('show');
        $.post('mstsupplier_edit.php', {
            idspl: $(this).attr('data-id'),
            btnproses: 'Update'
        }, function(html) {
            $('#form_mode').val('edit');
    ...
    $(document).on('click', '#delespl', function(e) {
        e.preventDefault();
        $("#ScrollableModal").modal('show');
        $.post('mstsupplier_del.php', {
            idspl: $(this).attr('data-id'),
            btnproses: 'Delete'
        }, function(html) {
            ...
            $('#detailedit').hide();
            $('#detailhapus').show();
            $('.modal-title').text('Delete Supplier');
    ...
    $('#formmodal').on('submit', (function(e) {
        ...
        let url = '';
        if (mode === 'add') url = 'mstsupplier_insert.php';
        else if (mode === 'edit') url = 'mstsupplier_update.php';
        else if (mode === 'delete') url = 'mstsupplier_delete.php';
        //alert(mode);
        $.ajax({
            url: url,
            type: "POST",
            ...
            success: function(response) {
                ...
                $('#addspl').focus(); // atau elemen lain di luar modal
                if (mode === 'add') {
